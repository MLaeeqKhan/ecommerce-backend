...
router.post('/postProduct',upload.single('image'), async (req, res) => {
    const { productName, productDescription, stockQuantity, productPrice,category } = req.body;
    ...
    try {
        if (!image || !productName || !productDescription || !productPrice || !stockQuantity || !category) {
...
router.get('/getCartData/:userId', async (req, res) => {
  ...
  try {
      ...
      res.status(500).send('getCartData: Internal Server Error while fetching cart data.');
  }
});

// Delete cart Product by cart Product ID
router.delete("/deleteCartProductsAfterPayment/:cartProductId", async (req, res) => {
...
router.delete("/deleteCartProducts/:userId", async (req, res) => {
  ...
  let userId ;
      const lineItems = products.map((product) => {
        const price = parseFloat(product.productId.productPrice.replace(/[^0-9.-]+/g, ""));
        const unitAmount = Math.round(price * 100);
        userId=product.userId;
        if (isNaN(unitAmount)) {
          throw new Error(`Invalid product price for product ID: ${product.productId._id}`);
        }
